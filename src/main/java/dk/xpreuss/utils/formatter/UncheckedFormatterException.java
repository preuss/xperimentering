...
public class UncheckedFormatterException extends RuntimeException {
	...
	public static UncheckedFormatterException invalidNumberOfArguments(int argsNum) {
		String msg = StringFormatterX.str(INVALID_NUMBER_OF_ARGUMENTS).arg("argsNum", argsNum).fmt();
		return new UncheckedFormatterException(msg);
	}

	public static UncheckedFormatterException argumentAlreadyExist(String arg) {
		String msg = StringFormatterX.str(ARGUMENT_ALREADY_DEFINED).arg("arg", arg).fmt();
		return new UncheckedFormatterException(msg);
	}

	public static UncheckedFormatterException invalidCharacterInParam(char c, int idx) {
		String msg = StringFormatterX.str(INVALID_CHARACTER_IN_PARAM_NAME).args("char", c, "idx", idx).fmt();
		return new UncheckedFormatterException(msg);
	}

	public static UncheckedFormatterException ioExceptionReadingFromFile(String strPath, Throwable t) {
		String msg = StringFormatterX.str(IO_EXCEPTION_READING_FROM_FILE).arg("strPath", strPath).fmt();
		return new UncheckedFormatterException(msg, t);
	}

	public static UncheckedFormatterException invalidArgumentName(Object argName) {
		String msg = StringFormatterX.str(INVALID_ARGUMENT_NAME_NULL_OR_EMPTY, "arg", argName).fmt();
		return new UncheckedFormatterException(msg);
	}

	public static UncheckedFormatterException invalidStateException(StringFormatterX.State state) {
		String msg = StringFormatterX.str(INVALID_STATE_EXCEPTION, "state", state).fmt();
		return new UncheckedFormatterException(msg);
	}

	public static UncheckedFormatterException invalidPositionalArgumentValue(Integer index) {
		String msg = StringFormatterX.str(INVALID_POSITIONAL_ARGUMENT, "index", index).fmt();
		return new UncheckedFormatterException(msg);
	}
}