package dk.xpreuss.utils.preuss;
...
public final class StringJoiner {
	...
	public String toString() {
		...
		if (size == 0) {
			...
			elements = EMPTY_STRING_ARRAY;
		}

		return stringJoin(prefix, suffix, delimiter, elements, size);
	}
	...
	public StringJoiner add(CharSequence newElement) {
		...
		len = checkAddLength(len, element.length());
	...
	public <T> StringJoiner addTrueElseOther(CharSequence prefix, T newElement, CharSequence newOtherElement,
		...
		return testIfNewElement.apply(newElement) ?
				addContatenated(prefix, String.valueOf(newElement)) :
				addContatenated(prefix, newOtherElement);
	}

	public StringJoiner addContatenated(CharSequence... newElements) {
	...
	private void compactElements() {
		int sz = size;
		if (sz > 1) {
			//elements[0] = String.join("", "", delimiter, elements, sz);
			elements[0] = String.join(delimiter, elements);
			Arrays.fill(elements, 1, sz, null);
			size = 1;
		}
	}
	...
	private static String stringJoin(String prefix, String suffix, String delimiter, String[] elements, int size) {
		String[] compressedElements = new String[size];
		System.arraycopy(elements, 0, compressedElements, 0, size);
		return prefix + String.join(delimiter, compressedElements) + suffix;
		//return String.join(prefix, suffix, delimiter, elements, size);
	}

	public StringJoiner addIfNotEmpty(String prefix, Object element) {
		if(element != null && String.valueOf(element).length() > 0) {
			return addContatenated(prefix, String.valueOf(element));
		}
		return this;
	}
}
